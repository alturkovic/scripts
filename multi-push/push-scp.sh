...
push() {
  ...
  if scp -q -P ${_port} ${_source_directory}${_file} ${_user}@${_host}:${_target_directory}${_file}.tmp; then
    echo "INFO - Copy successful for ${_file} @ ${_host}, removing .tmp extension"

    # remove temporary extension to mark copy as complete
    if ssh -l ${_user} ${_host} "mv ${_target_directory}${_file}.tmp ${_target_directory}${_file}"; then
      ...
      echo "ERROR - Rename failed for ${_file} @ ${_host}"
    fi

  else
    echo "ERROR - Copy failed for ${_file} @ ${_host}"
  fi
}
